...
contains
    ...
    subroutine pt_three_phase_from_pt_two_phase(ncomp, pt_2ph, pt_3ph)
        ...
        integer, intent(in):: ncomp !! Incipient component number
        type(envelope), intent(in) :: pt_2ph !! Initial PT envel
        type(PTEnvel3), intent(out) :: pt_3ph !! Output envelope
        real(pr) :: p, v, t
        real(pr) :: lnphix(nc), lnphiy(nc), lnphiz(nc), &
            phase_x(nc), phase_y(nc), beta, &
            x(2*nc+3), lnKx(nc), lnKy(nc)
        ...
        call pt_envelope_three_phase(X, 2*nc+2, 0.1_pr, pt_3ph)
    end subroutine pt_three_phase_from_pt_two_phase
end program main
